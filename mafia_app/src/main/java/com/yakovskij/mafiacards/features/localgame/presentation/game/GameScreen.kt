    ...
    Scaffold { innerPadding ->
        Box(modifier = Modifier.padding(innerPadding)) {
            when (uiState.phase) {
                ...
                GamePhase.DAY_DISCUSSION -> NightResultsScreen(onNextPhase = gameViewModel::advancePhase)
                GamePhase.VOTING -> VotingScreen(onNextPhase = gameViewModel::advancePhase)
                GamePhase.VOTED -> VoteResultsScreen(onNextPhase = gameViewModel::advancePhase)
